...
- Animation connector: choose a model clip, a **Start / End frame** range within one combined timeline, **COLLATERAL BUILT-IN**, or **NONE**, separately for each action. The independent **Layer procedural movement** switch controls first-person mouse-look sway, walk bob, inertia, and sprint/run carry without replacing authored fire or reload clips. Frame numbers are the imported Babylon timeline (which can differ from the source application's FPS). A missing explicitly selected clip does not silently enable a procedural action animation. The original G17 declares its Scene timeline ranges explicitly while retaining procedural first-person movement.
