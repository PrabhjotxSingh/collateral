...
import { RULES,ROUND_LIMITS,canStartMatch,type Team } from '../../shared/rules.js';
...
export class TacticalRoom extends Room<GameState> {
  ...
  async onCreate(options:any){
    ...
    this.onMessage('round-limit',(client,value)=>this.guard(client,()=>this.setRoundLimit(client,value)));
    this.onMessage('map-selection',(client,value)=>this.guard(client,()=>this.setMap(client,value)));
    this.onMessage('back-lobby',client=>this.guard(client,()=>this.backToLobby(client)));
  }
  ...
  protected startMatch(client:Client){
    ...
    this.lock();this.state.mapId=this.state.mapChoice==='random'?MAPS[Math.floor(Math.random()*MAPS.length)].id:this.state.mapChoice;this.state.phase='prep';this.state.round=1;this.state.remaining=RULES.prepSeconds;this.updateListing();
  }
  protected setRoundLimit(client:Client,value:unknown){
    if(client.sessionId!==this.state.hostId||this.state.phase!=='waiting')throw new Error('Only the host can change match length.');
    if(typeof value!=='number'||!(ROUND_LIMITS as readonly number[]).includes(value))throw new Error('Choose best of 3, 5, 7, or 9.');
    this.state.roundLimit=value;
  }
  protected setMap(client:Client,value:unknown){
    if(client.sessionId!==this.state.hostId||this.state.phase!=='waiting')throw new Error('Only the host can select a map.');
    if(value!=='random'&&(typeof value!=='string'||!MAPS.some(map=>map.id===value)))throw new Error('That map is not installed.');
    this.state.mapChoice=value;
  }
  protected backToLobby(client:Client){
    if(client.sessionId!==this.state.hostId||!['finished','abandoned'].includes(this.state.phase))throw new Error('Only the host can return everyone to the lobby.');
    this.unlock();const s=this.state;s.phase='waiting';s.round=0;s.scoreA=0;s.scoreB=0;s.remaining=0;s.winner='';s.reason='';
    for(const [id,p] of s.players){
      if(!p.connected){s.players.delete(id);continue;}
      p.health=RULES.health;p.ammo=RULES.magazine;p.reserve=RULES.reserve;p.reloading=false;p.sprint=false;p.crouch=false;p.ads=false;p.vx=0;p.vy=0;p.vz=0;
    }
    this.updateListing();
  }
  ...
  async onLeave(client:Client,consented:boolean){
    ...
    if(!consented&&sessions.get(identity.token)===identity)try{await this.allowReconnection(client,RULES.reconnectSeconds);if(sessions.get(identity.token)!==identity){client.leave(4001);throw new Error('Session ended');}p.connected=true;sessions.attach(identity,`match:${this.roomId}`);this.onReconnected(p);this.updateListing();return;}catch{}
