...
export function roundWinner(
  players: Iterable<Pick<PlayerState, "team" | "health" | "connected">>,
  _round: number,
  expired: boolean,
): Team | "draw" | undefined {
  let a = 0,
...
export function traceShot(
  ...
  yaw: number,
  pitch: number,
  maxRange: number = RULES.maxRange,
) {
  const origin = {
    x: shooter.x,
    y: shooter.y + (shooter.crouch ? RULES.crouchEyeHeight : RULES.eyeHeight),
    z: shooter.z,
  };
  const dir = direction(yaw, pitch);
  let distance: number = Math.min(maxRange, rayMap(origin, dir, map)),
  ...
  for (const player of players) {
    ...
    const headDistance = rayBox(origin, dir, {
      x: player.x,
      y: headY,
      z: player.z,
      w: RULES.headRadius * 2,
      h: RULES.headHeight,
      d: RULES.headRadius * 2,
    });
    const bodyHeight = height - RULES.headTopInset - RULES.headHeight;
    const bodyDistance = rayBox(origin, dir, {
      x: player.x,
...
export function spreadRadians(body: Body) {
  return (
    (body.grounded ? 0.0025 : 0.045) +
    (Math.hypot(body.vx, body.vz) > 0.2 ? 0.018 : 0) +
    (body.crouch ? -0.001 : 0) +
    (body.ads ? -0.001 : 0)
  );
}
