...
const oldMap={...MAPS[0],triangles:undefined,walls:[{x:0,y:1.5,z:0,w:5,h:3,d:8},{x:0,y:2,z:18,w:36,h:4,d:1}]};
const player=(id:string,team:'A'|'B',x:number,z:number)=>Object.assign(new PlayerState(),{id,team,x,z});
test('hitscan respects cover, body damage zone, heads and friendly blockers',()=>{
  const a=player('a','A',0,-10),b=player('b','B',0,10);
  assert.equal(traceShot(a,[a,b],oldMap,0,0).target,undefined);
  a.x=b.x=14;assert.equal(traceShot(a,[a,b],oldMap,0,0).headshot,true);
  assert.equal(traceShot(a,[a,b],oldMap,0,0.04).headshot,false);
  const friend=player('c','A',14,0);assert.equal(traceShot(a,[a,friend,b],oldMap,0,0).target,undefined);
});
...
test('movement is speed capped, collision bounded and held jump cannot bunny hop',()=>{
  const b=makeBody(14,0),i={...emptyInput(),forward:1,strafe:1};for(let n=0;n<60;n++)moveBody(b,i,1/60,oldMap);assert.ok(Math.hypot(b.x-14,b.z)<=RULES.walkSpeed+0.001);
  for(let n=0;n<600;n++)moveBody(b,{...i,strafe:0},1/60,oldMap);assert.ok(b.z<17.2);
  const jumping=makeBody(14,0);for(let n=0;n<180;n++)moveBody(jumping,{...emptyInput(),jump:true},1/60,oldMap);assert.equal(jumping.y,0);
...
test('callsign is released immediately; late old-session cleanup cannot remove its new owner',()=>{
 const r=new SessionRegistry(),old=r.claim('Gill',undefined);r.attach(old,'identity:x');assert.throws(()=>r.claim('gILL',undefined));
 r.detach(old,'identity:x');const next=r.claim('Gill',undefined);assert.notEqual(next.token,old.token);r.attach(next,'identity:y');
 r.release(old);r.detach(old,'match:x');assert.equal(r.get(next.token),next);assert.equal(r.get(old.token),undefined);assert.throws(()=>r.attach(old,'identity:z'));
});
test('draw rounds award no points, continue past round nine, and five decisive wins finish',()=>{
 ...
 for(const [id,team]of [['a','A'],['b','A'],['c','B'],['d','B']] as const)state.players.set(id,player(id,team,0,0));
 const engine=room as any;engine.prepRound();
 for(let round=1;round<=10;round++){
  assert.equal(state.round,round);assert.equal(state.phase,'prep');assert.ok(round%2===1?state.players.get('a')!.x<0:state.players.get('a')!.x>0,'neutral sides alternate');
  engine.simTime=engine.deadline;engine.tick(1/60);assert.equal(state.phase,'live');engine.simTime=engine.deadline;engine.tick(1/60);
  assert.equal(state.phase,'post');assert.equal(state.winner,'draw');assert.equal(state.scoreA,0);assert.equal(state.scoreB,0);
  engine.simTime=engine.deadline;engine.tick(1/60);
 }
 for(let win=1;win<=5;win++){
  engine.simTime=engine.deadline;engine.tick(1/60);state.players.get('d')!.health=0;
  engine.simTime=engine.deadline;engine.tick(1/60);assert.equal(state.scoreA,win);assert.equal(state.scoreB,0);
