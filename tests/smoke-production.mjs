...
try {
  ...
  const response = await fetch("http://127.0.0.1:2569/");
  assert.equal(response.status, 200);
  const html = await response.text();
  assert.ok(html.includes("<title>Collateral</title>"));
  for (const match of html.matchAll(/(?:src|href)="(\/assets\/[^\"]+)"/g)) {
    const response = await fetch(`http://127.0.0.1:2569${match[1]}`);
    assert.equal(response.status, 200, match[1]);
  }
  const wasm = (await readdir("client/dist/assets")).find((name) =>
    name.endsWith(".wasm"),
  );
  assert.ok(wasm);
  const bytes = await (
    await fetch(`http://127.0.0.1:2569/assets/${wasm}`)
  ).arrayBuffer();
  assert.equal(new DataView(bytes).getUint32(0, true), 0x6d736100);
  assert.equal(
  ...
  for (const path of [
    ...
    const response = await fetch(`http://127.0.0.1:2569/assets/${path}`);
    assert.equal(response.status, 200, path);
    const served = Buffer.from(await response.arrayBuffer());
    assert.equal(served.readUInt32LE(0), 0x46546c67, path);
    assert.deepEqual(
      served,
      await readFile(`client/public/assets/${path}`),
      "served model must match bundled bytes",
    );
  }
  for (const path of ["sounds/glock-shot.ogg", "sounds/reload.ogg"]) {
    ...
    assert.equal(response.status, 200, path);
    const served = Buffer.from(await response.arrayBuffer());
    assert.equal(served.subarray(0, 4).toString(), "OggS");
    assert.deepEqual(served, await readFile(`client/public/assets/${path}`));
  }
  const hitResponse = await fetch(
    "http://127.0.0.1:2569/assets/sounds/hitmarker.mp3",
  );
  assert.equal(hitResponse.status, 200, "sounds/hitmarker.mp3");
  const hitBytes = Buffer.from(await hitResponse.arrayBuffer());
  assert.ok(
    hitBytes.subarray(0, 3).toString() === "ID3" || hitBytes[0] === 0xff,
    "valid hitmarker MP3",
  );
  assert.deepEqual(
    hitBytes,
  ...
  child.kill("SIGTERM");
  const t = setTimeout(() => child.kill("SIGKILL"), 1500);
  t.unref();
}
