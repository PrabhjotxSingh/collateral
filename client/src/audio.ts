...
export class TacticalAudio {
  ...
  private async load(){
    await Promise.all((['shot','step','reload'] as Cue[]).map(async kind=>{try{
  ...
  play(kind:Cue,position?:Vec,volume=1){
    ...
    const cueVolume=Math.max(0,Math.min(1,volume));gain.connect(destination);const now=ctx.currentTime;const duration=kind==='shot'?0.12:kind==='step'?0.075:0.18;
    let source:AudioBufferSourceNode|OscillatorNode;
    if(recorded){source=ctx.createBufferSource();source.buffer=recorded;gain.gain.value=(kind==='shot'?0.65:0.4)*cueVolume;source.connect(gain);source.start();}
    else{
