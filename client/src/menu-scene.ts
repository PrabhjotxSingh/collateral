import {Engine,Scene,UniversalCamera,Camera,Vector3,Color3,Color4,HemisphericLight,DirectionalLight,PointLight,TransformNode} from '@babylonjs/core';
...
export class MenuScene {
  ...
  constructor(canvas:HTMLCanvasElement){
    ...
    const hover=new PointLight('menu-hover-red',new Vector3(.45,1.45,1),this.scene);hover.diffuse=new Color3(.9,.055,.035);hover.intensity=0;hover.range=3.2;
    let pointerX=.75,pointerY=.45;
    window.addEventListener('pointermove',event=>{pointerX=event.clientX/Math.max(1,innerWidth);pointerY=event.clientY/Math.max(1,innerHeight);});
    ...
    this.engine.runRenderLoop(()=>{
      ...
      const overOperator=pointerX>.5,targetGlow=overOperator?5.2:0;hover.intensity+=(targetGlow-hover.intensity)*Math.min(1,dt*7);hover.position.set((pointerX-.5)*2.4,2.15-pointerY*1.45,1.05);
