...
export interface Settings {sensitivity:number;fov:number;master:number;sfx:number;crosshairMode:'dynamic'|'static';crosshairGap:number;keys:Record<Action,string>}
export const defaults:Settings={sensitivity:1,fov:95,master:0.7,sfx:0.8,crosshairMode:'dynamic',crosshairGap:6,keys:{forward:'KeyW',back:'KeyS',left:'KeyA',right:'KeyD',jump:'Space',crouch:'KeyC',reload:'KeyR',fire:'Mouse0',ads:'Mouse2',sprint:'ShiftLeft'}};
export function loadSettings():Settings{
  try{const s=JSON.parse(localStorage.getItem('collateral.settings')??'{}');const clamp=(v:unknown,fallback:number,min:number,max:number)=>typeof v==='number'&&Number.isFinite(v)?Math.max(min,Math.min(max,v)):fallback;
    ...
    return {sensitivity:clamp(s.sensitivity,1,0.1,3),fov:clamp(s.fov,95,80,110),master:clamp(s.master,0.7,0,1),sfx:clamp(s.sfx,0.8,0,1),crosshairMode:s.crosshairMode==='static'?'static':'dynamic',crosshairGap:clamp(s.crosshairGap,6,2,18),keys};
