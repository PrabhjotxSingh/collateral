...
try {
  game = new Game(document.querySelector("#game")!, net, settings);
} catch {
  ui.message(
    "A browser with WebGL support is required to play. Enable hardware acceleration and reload.",
  );
}
try {
  const menu = new MenuScene(document.querySelector("#menu-scene")!);
  void Promise.race([
    menu.ready,
  ...
  ui.message("The menu character could not be displayed.");
}
let downloadingMapId: string | undefined;
net.addEventListener("match", () => {
  const path = settings.primary
    ? `primary/${settings.primary}`
    : `secondary/${settings.secondary}`;
  if (net.weapons.some((weapon) => weapon.path === path))
    net.send("weapon", path);
});
    ...
    if (downloadingMapId !== state.mapId) {
      ...
      void net
        ...
        .catch((error) => {
          ...
          setLoading(false);
          ui.message((error as Error).message);
        });
    }
    return;
  }
