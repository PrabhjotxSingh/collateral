...
import {Engine,Scene,Camera,UniversalCamera,Vector3,HemisphericLight,DirectionalLight,ShadowGenerator,MeshBuilder,StandardMaterial,ShaderMaterial,Color3,Color4,Mesh,TransformNode,VertexData} from '@babylonjs/core';
...
export class Game {
  ...
  constructor(private canvas:HTMLCanvasElement,private network:Network,settings:Settings){
    ...
    const sky=MeshBuilder.CreateSphere('blue-daylight-sky',{diameter:180,segments:24},this.scene);sky.infiniteDistance=true;sky.isPickable=false;sky.applyFog=false;
    const skyMaterial=new ShaderMaterial('daylight-gradient',this.scene,{vertexSource:'precision highp float;attribute vec3 position;uniform mat4 worldViewProjection;varying float elevation;void main(){elevation=max(0.0,position.y/90.0);gl_Position=worldViewProjection*vec4(position,1.0);}',fragmentSource:'precision highp float;varying float elevation;void main(){gl_FragColor=vec4(mix(vec3(.72,.85,.98),vec3(.12,.43,.86),pow(elevation,.65)),1.0);}'},{attributes:['position'],uniforms:['worldViewProjection']});skyMaterial.backFaceCulling=false;sky.material=skyMaterial;
    ...
    this.effects=new ShotEffects(this.scene);this.dev=new DevTools(this.scene,()=>this.actorAssets.values());
    ...
    window.addEventListener('mousemove',e=>{if(!this.locked||!this.active)return;const factor=0.002*this.settings.sensitivity*(this.held('ads')?0.72:1);this.yaw=(this.yaw+e.movementX*factor)%(Math.PI*2);this.pitch=Math.max(-1.5,Math.min(1.5,this.pitch+e.movementY*factor));});
  ...
  private loadMap(id:string){
    ...
    this.mapId=id;this.mapModel?.dispose();for(const mesh of this.environment)mesh.dispose();this.environment=[];
    ...
    const root=this.mapModel=new TransformNode('map-model',this.scene);root.position.y=map.offsetY??0;const placeholders=[...this.environment];
  ...
  update(state:GameView){
    this.state=state;this.active=state.phase!=='waiting'&&state.phase!=='finished';
  ...
  private frame(dt:number){
    ...
    if(!this.state||!this.active){this.gun.setEnabled(false);document.body.classList.remove('is-ads');return;}
    ...
    const sprint=this.locked&&this.held('sprint')&&!this.cancelSprint&&!this.held('ads')&&!this.held('crouch')&&!me.reloading&&this.held('forward');
    this.elapsed+=dt;
    ...
    if(target){this.camera.position=Vector3.Lerp(this.camera.position,new Vector3(target.x,target.y+(target.crouch?RULES.crouchEyeHeight:RULES.eyeHeight),target.z),1-Math.exp(-18*dt));this.camera.rotation.set(target.id===me.id?this.pitch-this.kick:target.pitch,target.id===me.id?this.yaw:target.yaw,0);}
    ...
    for(const [id,mesh]of this.players){
      ...
      if(!dead){actor?.clips.play(action);if(p.reloading){if(!this.actorReloading.has(id)){this.actorReloading.add(id);actor?.combat?.play('reload',false,RULES.reloadSeconds);}}else if(this.actorReloading.delete(id))actor?.combat?.stop();}
  ...
  private shot(shot:ShotEvent){
    ...
    if(local){if(!this.prediction.acknowledge(shot.shotId))this.localShot();if(shot.hit)window.dispatchEvent(new CustomEvent('game-hit',{detail:shot.headshot}));}
